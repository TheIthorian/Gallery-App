...
class UserProfile ():
    ...
    def constructUserProfile(self, userId):
        ...
        if userId > 0:
            result = server.serverConnection.runQuery(
                "User", "GetUserSecurityProfile", {'UserId': userId})
    ...
    def registerSession(self, sessionId):
        ...
        try:
            inputs = {'SessionID': sessionId,
                      'UserId': self.userId, 'UserId2': self.userId}
            server.serverConnection.runQuery(
                "User", "CreateUserSession", inputs)
    ...
    def sessionLogin(self, sessionId):
        try:
            result = server.serverConnection.runQuery(
                "User", "GetUserSession", {'SessionID': sessionId})
    ...
    def userLogin(self, username, password, sessionId):
        ...
        try:
            result = server.serverConnection.runQuery(
                "User", "ValidatePasswordForUser", {'Username': username})
        except Exception as e:
            return {
                'verified': False,
    ...
    def userLogout(self):
        ...
        try:
            server.serverConnection.runQuery(
                "User", "DeleteUserSession", {'UserId': self.userId})
        except:
            print("SQL Error")
            return False
...
def register(email, username, password, sessionId):
    ...
    try:
        ...
        userId = server.serverConnection.runInsertQuery(
            "User", "UsrInsert", inputs)
